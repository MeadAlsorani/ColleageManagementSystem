...
{
    public class StudentDetailsRequestHandler : BaseRequestHandler, IRequestHandler<StudentDetailsRequest, UpdateStudentDto>
    {
        public StudentDetailsRequestHandler(IStudentRepository student, IMapper mapper) : base(student, mapper)
        {
        }
        public async Task<UpdateStudentDto> Handle(StudentDetailsRequest request, CancellationToken cancellationToken)
        {
            var record = await repository.Get(request.Id);
            return mapper.Map<UpdateStudentDto>(record);
        }
    }
}
